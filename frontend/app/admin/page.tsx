...
export default function AdminPage() {
  ...
  // Active Service Tab (Postgres, Redis, Wiremock, LocalStack - No "All" tab)
  const [activeTab, setActiveTab] = useState<ServiceType>(CONSTANTS.DEFAULT_SERVICE_TYPE);
  ...
  const activeTheme = SERVICE_THEME[activeTab];
  const ActiveIcon = activeTheme.icon;
  const activeConnections = (connections ?? []).filter((c) => c.type === activeTab);
  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      {/* Service Category Navigation Tabs (Postgres, Redis, Wiremock, LocalStack) */}
