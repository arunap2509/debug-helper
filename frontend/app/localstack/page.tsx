...
const CONSTANTS = {
  ...
  LABELS: {
    ...
    NO_QUEUES: "No SQS queues found on this LocalStack instance.",
    ...
    <div className="space-y-6 pb-12">
        ...
        <div className="space-y-4">
          ...
          {buckets?.length === 0 && (
            <div className="rounded-2xl border border-dashed border-slate-800 bg-slate-900/20 p-12 text-center">
              <p className="text-xs text-slate-500">No S3 buckets provisioned.</p>
            </div>
          )}
        </div>
      )}
      ...
      {tab === "ssm" && (
        <Card className="overflow-hidden p-0 border-slate-800/90 bg-slate-900/50 backdrop-blur-xl">
          ...
          {ssm?.length === 0 && <div className="p-6 text-center text-xs text-slate-500">No SSM parameters provisioned.</div>}
        </Card>
      )}
    </div>
  );
